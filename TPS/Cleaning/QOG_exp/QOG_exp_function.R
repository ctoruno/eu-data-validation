...
QOG_exp_clean <- function(df){
  ...
  normalized[nrow(normalized)+1,]<- list("Luxembourg", NA_real_, NA_real_, NA_real_)
  
  aggregate<- normalized%>%
    filter(cname %in% cntry)%>%
    arrange(cname)
  ...
  clean <- aggregate%>%
    mutate(Country = case_when(is.na(Country) ~ 
                                 deframe(tibble(cntry, nuts))[cname], 
                               TRUE ~ Country))%>%
