...
if (!("red" %in% colnames(flagsoutput))){

  flagsoutput$red<- rep(0, nrow(flagsoutput))
...
if (!("red" %in% colnames(flagsoutputsub))){

  flagsoutputsub$red<- rep(0, nrow(flagsoutputsub))
} 
...
anuts<- thresholdnuts%>%
  ...
  arrange(Sub_Pillar)
anuts$Direction<- ifelse(anuts$GPP_datapoint-anuts$TPS_datapoint>0, "Positive", ifelse(anuts$GPP_datapoint-anuts$TPS_datapoint<0, "Negative", "No Change"))
anuts$Flag<- ifelse(anuts$Flag == "red", "Red", ifelse(anuts$Flag == "green", "Green", NA))
...
colnames(anuts)<- c("Sub Pillar", "NUTS", "GPP Indicator", "Flag", "GPP Question",  "TPS Question", "Match Level", "TPS Source", "TPS Year", "GPP Score", "TPS Country Score", "Difference", "Direction", "Pillar")
...
enuts<- expertsnuts%>%
  select(Sub_Pillar, NUTS, `2023  EU Questionnaire`, Flag, Description, TPS_Question, Match, TPS_Source, TPS_Year, GPP_datapoint, TPS_datapoint, Difference,  Pillar )%>%
  arrange(Sub_Pillar)
enuts$Direction<- ifelse(enuts$GPP_datapoint-enuts$TPS_datapoint>0, "Positive", ifelse(enuts$GPP_datapoint-enuts$TPS_datapoint<0, "Negative", "No Change"))
enuts$Flag<- ifelse(enuts$Flag == "red", "Red", ifelse(enuts$Flag == "green", "Green", NA))

enuts<- enuts%>%
  select(Sub_Pillar, NUTS, `2023  EU Questionnaire`, Flag, Description, TPS_Question, Match, TPS_Source, TPS_Year, GPP_datapoint, TPS_datapoint, Difference, Direction,  Pillar )

colnames(enuts)<- c("Sub Pillar", "NUTS", "GPP Indicator", "Flag", "GPP Question",  "TPS Question", "Match Level", "TPS Source", "TPS Year", "GPP Score", "TPS Country Score", "Difference", "Direction", "Pillar")
...
bnuts<- timenuts%>%
  ...
  arrange(subpillar)
bnuts$Difference<- abs(bnuts$current_score- bnuts$previous_score)
...
colnames(bnuts)<- c("Sub Pillar", "NUTS", "GPP Indicator", "Flag", "GPP Question", "Comparison Year", "Current Score", "Previous Country Score", "Difference",  "Direction", "Pillar" )

bnuts$Flag<- ifelse(bnuts$Flag == "Red light", "Red", ifelse(bnuts$Flag == "Green light", "Green", NA))
...
```{r}
popsum <- anuts[grepl(paste(spmatches, collapse="|"), anuts$`Sub Pillar`), ]
popsum <- popsum%>%
  select(-Pillar)%>%
  arrange(`Sub Pillar`, NUTS)
ttsum  <- bnuts[grepl(paste(spmatches, collapse="|"), bnuts$`Sub Pillar`), ]
ttsum  <- ttsum%>%
  select(-Pillar)%>%
  arrange(`Sub Pillar`, NUTS)
expsum <- enuts[grepl(paste(spmatches, collapse="|"), enuts$`Sub Pillar`), ]
expsum <- expsum%>%
  select(-Pillar)%>%
  arrange(`Sub Pillar`, NUTS)
