...
time2<-left_join(time, threshold, by = join_by(variable == "GPP_Variable_Name"), relationship = "many-to-many")

time<- time2%>%
  select("country","variable", "ttestResult","current_score","previous_score", "warning","curr_year", "prev_year", Pillar, Sub_Pillar )%>%
  distinct()
```
...
1. If 25% or more of the indicators have a threshold difference above 0.25 (red flag), or
  
2. If 50% or more of the indicators have a threshold difference above 0.10 (yellow flag)
...
Furthermore, the structural analysis describes the proportion of respondents who answered "Don't Know" or "Prefer Not to Say" in at least 10% of questions. This is helpful in the pretest specifically because we want to minimize the number of missing or NA values we receive. 
...
flagsoutput$Label<- ifelse(flagsoutput$red> flagsoutput$sum/4, "Significantly Different", ifelse(flagsoutput$red+flagsoutput$yellow > flagsoutput$sum/2, "Significantly Different", ifelse(flagsoutput$yellow > flagsoutput$sum/2, "Significantly Different", "Not Significantly Different")))
...
warningsoutput$Label<- ifelse(warningsoutput$`Red light`> warningsoutput$sum/4, "Significantly Different", ifelse(warningsoutput$`Red light`+warningsoutput$`Yellow light` > warningsoutput$sum/2, "Significantly Different", ifelse(warningsoutput$`Yellow light` > warningsoutput$sum/2, "Significantly Different", "Not Significantly Different")))
...
Below are two tables summarizing each of the validation analyses to provide further details about the issues we have flagged. If you are interested in exploring which specific variables are flagged, please see the Appendix.
...
In the pretest data, 100% of the respondents answered at least 10% of the questions with DK/PNTS.
  
Below is a table listing all of the variables where over 7.5% of respondents answered DK/PNTS.

```{r}
missingdf<- tibble(Pillar = c("q44j_G2", "q59f", "q4d", "q4e" , "q39d_G1" , "q54_G2"  , "q56_G2",  "q43f_G1"), "Proportion Missing" = c("12%", "10%", "15%", "20%", "11%", "12%", "14%", "10%"))

knitr::kable(missingdf, caption = "Variables with more than 7.5% DK/PNTS")%>%
  kable_classic(html_font = "Arial", )%>%
...
b<- appendixtt%>%
  select(Sub_Pillar, `2023  EU Questionnaire`, warning, Description, prev_year, previous_score, current_score,   ttestResult, Pillar )%>%
  arrange(Sub_Pillar)

colnames(b)<- c("Sub Pillar", "GPP Indicator", "Flag", "GPP Question", "Comparison Year", "Previous Score", "Current Score", "P value",   "Pillar" )
...
threshex<- "In the threshold difference, a red flag indicates a difference of 0.25 or more. A yellow flag indicates a difference between 0.10 and 0.25. A green flag indicates a difference of less than 0.10."
```
...
#### Threshold (External) Analysis {.tabset}
